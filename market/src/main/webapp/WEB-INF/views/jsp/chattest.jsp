...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/resources/css/header.css?ver=1.1 ">
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/resources/css/chat.css?ver=1.1 ">
<script
	src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
<script
	src="http://cjhwebsocket.cafe24app.com:80/socket.io/socket.io.js"></script>
...
<body>
	...
	<header>
		<div>
			<label>MemberNO : </label><input id="memberno" type="text"
				value="${boardMemberno} "><br> <label>RoomNO :
			</label><input id="roomno" type="text" value="${boardBoardno}"><br>
			<label>LoginMemberNO : </label><input id="loginmemberno" type="text"
				value="${loginMemberno}"><br> <label>MyProfile
				: </label><input id="myprofilename" type="text"
				value="${sessionScope.memberVO.profile}"><br> <label>Profile
				: </label><input id="profilename" type="text" value=""><br> <input
				type="hidden" id="location" value="0">
		</div>
		...
		<nav>
			<div class="header_chatting_div">
				...
				<div class="header_login_div">
					...
					<span>최정환</span> <img class="header_login_div_down"
						src="https://cdn-icons.flaticon.com/png/512/2985/premium/2985150.png?token=exp=1637135271~hmac=3f62a84e9d5a4112839b78e572418d09">
				</div>
	...
	<div class="chat_main">
		...
		<div id="chat_user">
			...
			<div id="chat_user_list" class="chat_user_list">
				...
				<div id="chat_user_readm" class="chat_user_readm">
					...
					<div class="readm_img_span">
						안읽은 메시지만 보기 <img id="readm_img" class="readm_img"
							src="http://cjhftp.dothome.co.kr/ico/tick2.png">
					</div>

				</div>
				<!-- 여기서 부터 사람 한명 -->
				<c:forEach items="${mychatList}" var="chatVO" varStatus="status">
					<input type="hidden" id="user${status.count}"
						name="user${status.count}" value="${chatVO.user2}">


					<div class="chat_user_list_room">
						<a id="chatroomno" class="chatroom${chatVO.chatroomno }"> <span
							class="boardMemberno" style="display: none;">${chatVO.user2}</span>
							<input type="hidden" id="boardMemberno" name="boardMemberno"
							value="${chatVO.user2}"> <span class="boardRoomno"
							style="display: none;">${chatVO.chatroomno}</span>
							<div class="chat_user_list_room_m">
								<div class="chat_user_list_room_m_img_div">
									<%--                                   <c:choose>
                                    <c:when test="${sessionScope.memberVO==null}"> --%>
									<img
										src="http://cjhftp.dothome.co.kr/${chatVO.user2}/profile/${chatVO.profile}">
									<span id="userprofile" style="display:none;" >${chatVO.profile}</span>
									<%--                              </c:when>
                                 </c:choose> --%>
								...
								<div class="chat_user_list_room_m_center_div">
									<div class="chat_user_list_room_m_member_div">
										<span>${chatVO.name }</span> <span> · </span> <span>${chatVO.datetime }
										</span>
									</div>
									<br>
									<div class="chat_user_list_room_m_message_div">
										<span> ${chatVO.chatmessage } </span>
									</div>
									<c:choose>
		...
		<div id="chat_box">
			...
			<div id="chat_box_ms" class="chat_box_ms"></div>
...
<script type="text/javascript"
	src="//dapi.kakao.com/v2/maps/sdk.js?appkey=25eb93196f96f5e9cacf100023363c56"></script>
<script>
	$("#msg_location")
			.click(
					function() {
						...
						window
								.open(
										"chatlocation?user1=" + user1
												+ "&user2=" + user2
												+ "&chatroomno=" + chatroomno
												+ "&locationjsp=" + location
												+ "&calendarLocationno="
												+ calendarLocationno,
										"_blank",
										"width=1100, height=700,toolbar=no,location=no,resizable=no,left=30,top=30,menubar=no");
		...
		$('.chat_user_readm div').on(
						{
							'click' : function() {
								if (jQuery('#readm_img').attr("src") == 'http://cjhftp.dothome.co.kr/ico/tick.png') {
													...
													$(data).each(
															function(index) {
																		var maindiv = $('<div class="chat_user_list_room">'
																				...
																				+ '<span class="boardMemberno" style="display:none;">'
																				+ this.user2
																				+ '</span>'
																				+ '<span class="boardRoomno" style="display:none;">'
																				+ this.chatroomno
																				+ '</span>'
																				+ '<span class="profilename" style="display:none;">'
																				+ this.profile
																				+ '</span>'
																				+ '<div class="chat_user_list_room_m">'
																				+ '<div class="chat_user_list_room_m_img_div">'
									...
									$('#readm_img')
											.attr('src','http://cjhftp.dothome.co.kr/ico/tick.png');
									//안읽은 메세지만 나옴    
													...
													if ($("#roomno").val() == jbSplit[1]) {
														if ($("#memberno").val() == jbSplit[2]) {
															if (jbSplit[5] == 1) {
																...
																$('<div class="a"><div class="chat_box_p_div"><img src="http://cjhftp.dothome.co.kr/'
																				+ jbSplit[2]
																				+ '/profile/'
																				+ $("#profilename").val()
																				+ '" ></div><div class="chat_box_m_div" > <span>'
																				+ jbSplit[3]
																				+ '</span></div><div class="chat_box_m_div_time_div"><span class="chat_box_m_div_time" >'
																				+ jbSplit[4]
																...
																$(
																		' <div class="c"><div class="chat_box_p_div"><img src="http://cjhftp.dothome.co.kr/'
																				...
																				+ $('#profilename').val()
																				+ '">'
																				+ '</div><div class="chat_box_m_div" ><div>일정이 공유되었어요.</div><div id="staticMap'+jbSplit[10]+'" class="chat_box_m_div_map"></div>'
															...
															if ($("."+ $("#roomno").val()+ "-"+ $("#memberno").val()).html() == 1) {
																...
																if (jbSplit[5] == 1) {
																	$('<div class="b"><div class="chat_box_m_div_time_div"><span class="chat_box_m_div_time" >'
																					+ jbSplit[4]
																					+ '</span></div><div class="chat_box_m_div" ><span>'
																					+ jbSplit[3]
																					+ '</span></div><div class="chat_box_p_div"><img src="http://cjhftp.dothome.co.kr/'
																					+ jbSplit[2]
																					+ '/profile/'
																					+ $("#myprofilename").val()+ '" ></div></div>').appendTo("#chat_box_ms");
																} else if (jbSplit[5] == 2) {
																	$(
																			' <div class="d"><div class="chat_box_m_div_time_div"><span class="chat_box_m_div_time" >'
																					+ jbSplit[4]
																					+ '</span></div>'
																					+ '<div class="chat_box_m_div" ><div> 일정이 공유되었어요.</div><div id="staticMap'+jbSplit[10]+'" class="chat_box_m_div_map">'
																...
																//상대가 내방에 접속중이 아님
																if (jbSplit[5] == 1) {
																	$('<div class="b"><div class="chat_box_m_div_read_div"><span class="chat_box_m_div_read_div_span">1</span></div><div class="chat_box_m_div_time_div"><span class="chat_box_m_div_time" >'
																					+ jbSplit[4]
																					+ '</span></div><div class="chat_box_m_div" ><span>'
																					+ jbSplit[3]
																					+ '</span></div><div class="chat_box_p_div"><img src="http://cjhftp.dothome.co.kr/'
										...
										success : function(data) {
											...
											$(data).each(function(index) {
																...
																if (loginmemberno == this.seller) {
																	...
																	//console.log("dong "+this.locationno);
																	if (this.readst == 0) {
																		if (this.locationno != 0) {

																			const location = this.location;
																			//console.log("location:"+location);
																			const jbSplit = location
																					.split('//');
																			...
																			var staticMapContainer = document
																					.getElementById('staticMap'
																							+ this.locationno), // 이미지 지도를 표시할 div  
																			staticMapOption = {
																				center : new kakao.maps.LatLng(
																						jbSplit[2],
																						jbSplit[1]), // 이미지 지도의 중심좌표
																				level : 3, // 이미지 지도의 확대 레벨
																				marker : marker
																			// 이미지 지도에 표시할 마커 
																			};
																			...
																			$('<div class="b"><div class="chat_box_m_div_read_div"><span class="chat_box_m_div_read_div_span"></span></div><div class="chat_box_m_div_time_div"><span class="chat_box_m_div_time" >'
																							+ this.datetime
																							+ '</span></div><div class="chat_box_m_div" ><span>'
																							+ this.chatmessage
																							+ '</span></div><div class="chat_box_p_div"><img src="http://cjhftp.dothome.co.kr/'+this.seller+'/profile/'+this.profile+'" ></div></div>').appendTo("#chat_box_ms");
																		}
																	...
																	if (this.locationno != 0) {
																		...
																		$('<div class="a"><div class="chat_box_p_div"><img src="http://cjhftp.dothome.co.kr/'+this.seller+'/profile/'+this.profile+'" ></div><div class="chat_box_m_div" > <span>'
																						+ this.chatmessage
																						+ '</span></div><div class="chat_box_m_div_time_div"><span class="chat_box_m_div_time" >'
																						+ this.datetime
																						+ '</span></div></div>').appendTo("#chat_box_ms");
